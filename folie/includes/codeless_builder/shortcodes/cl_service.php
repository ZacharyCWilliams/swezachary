		...
		$output .= '<div class="wrapper-form" '.$this->generateStyle('.cl_service > .icon_wrapper .wrapper-form').'>';
			...
			if( $media == 'type_icon' )
				...
				if( $media == 'type_svg' ){
					...
					if( function_exists( 'codeless_builder_f_get_contents' ) )
						$output .= codeless_builder_f_get_contents($svg_source);
				}
			else
				if( $media == 'type_custom' ){
					...
					$output .= '<img src="' . codeless_get_attachment_image_src( $img_id, $image_size ) .'" alt="'.esc_attr__('Placeholder Image', 'folie').'" />';
