...
if($video == 'self'){
		...
		$video_wrapper .= '     <video poster="'.$video_poster_image.'" muted="muted" preload="auto" '.((int)$row_video_loop ? "loop" : "").' autoplay="true">';
        ...
        $video_wrapper .= '<div class="video-section social-video"  data-stream="'.$video.'" style="opacity:0;">';
			...
			if ($video == 'youtube')
			    $video_wrapper .= '<iframe src="//www.youtube.com/embed/'.$video_youtube.'?rel=0&amp;wmode=transparent&amp;enablejsapi=1&amp;controls=0&amp;showinfo=0&amp;loop='.(int)$row_video_loop.'&amp;playlist='.$video_youtube.'"></iframe>';
	
	        if ($video == 'vimeo')
	            $video_wrapper .= '<iframe src="//player.vimeo.com/video/'.$video_vimeo.'?badge=0;api=1;background=1;autoplay=1;loop='.(int)$row_video_loop.'" width="640" height="360" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
