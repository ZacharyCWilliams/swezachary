...
class CodelessHeaderImporter{
	...
	public function process_install_header(){
		...
		if( $theme_mods && ! empty( $theme_mods ) ){
			...
			wp_send_json_error( array('message' => esc_html__('Options not loaded or files missed.', 'folie')) );
		}

		wp_send_json_success( array('message' => esc_html__('Header Successfully imported', 'folie')) );
	}
	...
	public function process_install_footer(){
		...
		wp_send_json_success( array('message' => esc_html__('Footer Successfully imported', 'folie')) );
	}
	...
	// Read the file that will be written
		public function read_file_header($file, $dir){
			...
			if ( file_exists($file) ) {
				...
				$this->message = esc_html__("File doesn't exist", "folie");
			}
...
class CodelessImporter{
	...
	public function process_import_widgets(){
		...
		if($options){
			...
			wp_send_json_success( array('message' => esc_html__('Widgets Successfully imported', 'folie')) );
			
		}else{
			wp_send_json_error( array('message' => esc_html__('Demo doesnt contain sidebar_widgets.txt file.', 'folie')) );
		}
	...
	public function process_import_options(){
		...
		if( $theme_mods && ! empty( $theme_mods ) ){
			...
			if( $options ){
				...
				wp_send_json_error( array('message' => esc_html__('Options not loaded or files missed.', 'folie'). $val) );
			}
		}
	...
	public function process_import_content(){
		...
		if(is_wp_error($value)){
			...
			wp_send_json_error( array('message' => esc_html__('Error. Content can\'t be installed.', 'folie') . $msg ) );
		}
		else {
			wp_send_json_success( array('message' => esc_html__('Content Successfully Installed', 'folie')) );
		}
	}
	...
	public function process_import_menus(){
		...
		if($menus_data){
			...
			wp_send_json_success( array('message' => esc_html__('Menu Successfully Installed', 'folie')) );

		}else{
			wp_send_json_error( array('message' => esc_html__('Error. Menu can\'t installed.', 'folie') ) );
		}
	}
	...
	public function ajax_plugins() {
			...
			$json['active'] = array(
						...
						'action2' => -1,
						'message' => esc_html__( 'Activating Plugin','folie' ),
			);
			...
			$json['install'] = array(
						...
						'action2' => -1,
						'message' => esc_html__( 'Installing Plugin','folie' ),
			);
			...
			if ( $json && ( !empty($json['active']['plugin']) || !empty($json['install']['plugin']) ) ) {
				...
				wp_send_json_success( array( 'message' => esc_html__( 'No plugins to install or activate', 'folie' ) ) );
			}
			exit;
