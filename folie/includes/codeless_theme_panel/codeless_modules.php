	...
	class CodelessModules {
		...
		public static function addPage() {
			if( function_exists( 'codeless_add_submenu_page' ) )
				codeless_add_submenu_page(
					'codeless-panel',
					esc_html__( 'Codeless Modules', 'folie' ),
					esc_html__( 'Codeless Modules', 'folie' ),
					'administrator',
		...
		public static function createPage() {
			...
			<div class="wrap">
				...
				<h2><?php esc_html_e( 'Modules', 'folie' ); ?></h2>

				<p><?php esc_html_e( 'In this Options Section you can define what modules you will use around Codeless Folie WordPress Theme.', 'folie' ); ?></p>
								...
								<td>
									<label>
										<input id="cl_custom_portfolio_overlay_color" type="checkbox" name="codeless_modules[cl_custom_portfolio_overlay_color]" <?php checked( codeless_get_mod( 'cl_custom_portfolio_overlay_color', false ) ); ?> />
											<?php esc_html_e( 'Individual Portfolio Overlay Color', 'folie' ); ?>
											<p class="description"><?php esc_html_e( 'By activating this module, a new option for each portfolio item will be created. From that moment you can add custom overlay color for each portfolio item.', 'folie' ); ?></p>
									</label>
								...
								<td>
									<label>
										<input id="cl_custom_header_boxed_per_page" type="checkbox" name="codeless_modules[cl_custom_header_boxed_per_page]" <?php checked( codeless_get_mod( 'cl_custom_header_boxed_per_page', false ) ); ?> />
											<?php esc_html_e( 'Custom Header Boxed Option for each page.', 'folie' ); ?>
											<p class="description"><?php esc_html_e( 'By activating this module, a new option for each page item will be created. From that moment you can active "header boxed" only for a specific page.', 'folie' ); ?></p>
									</label>
								...
								<td>
									<label>
										<input id="cl_custom_header_background_per_page" type="checkbox" name="codeless_modules[cl_custom_header_background_per_page]" <?php checked( codeless_get_mod( 'cl_custom_header_background_per_page', false ) ); ?> />
											<?php esc_html_e( 'Custom Header Background Option for each page.', 'folie' ); ?>
											<p class="description"><?php esc_html_e( 'By activating this module, a new option for each page item will be created. From that moment you can select "custom header background" for each page.', 'folie' ); ?></p>
									</label>
