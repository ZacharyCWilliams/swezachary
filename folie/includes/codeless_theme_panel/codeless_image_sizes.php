...
if ( ! class_exists( 'CodelessImageSizes' ) ) {
	...
	class CodelessImageSizes {
		...
		public function __construct() {
			...
			// Add Images
			add_action( 'init', array( 'CodelessImageSizes', 'addSizes' ), 1 );
		...
		public static function createPage() {
			...
			<div class="wrap">
				...
				<h2><?php esc_html_e( 'Image Sizes', 'folie' ); ?></h2>

				<p><?php esc_html_e( 'In this Options Section you can define all image sizes of site. Leave the width and height empty to display the full image. Leave empty the height attribute to disable the crop.', 'folie' ); ?></p>
				...
				<h2 class="nav-tab-wrapper codeless-image-sizes-admin-tabs" style="margin-top:20px;">
					...
					$tabs = apply_filters( 'codeless_image_sizes_tabs', array(
						'general' => esc_html__( 'General', 'folie' ),
						'blog'    => esc_html__( 'Blog', 'folie' ),
						'portfolio'    => esc_html__( 'Portfolio', 'folie' ),
						'team'    => esc_html__( 'Team', 'folie' ),
					) );

					
					$tabs['other'] = esc_html__( 'Other', 'folie' );
				...
				<form method="post" action="options.php">
					...
					<table class="form-table codeless-image-sizes-admin-table">
						<tr valign="top" data-codeless-section="general">
							<th scope="row"><?php esc_html_e( 'Image Resizing', 'folie' ); ?></th>
							<td>
								<fieldset>
									<label>
										<input id="codeless_optimize_image_resizing" type="checkbox" name="codeless_image_sizes[optimize_image_resizing]" <?php checked( codeless_get_mod( 'optimize_image_resizing', true ) ); ?>>
											<?php esc_html_e( 'Optimized Image Sizes', 'folie' ); ?>
						...
						<tr valign="top" data-codeless-section="general">
							<th scope="row"><?php esc_html_e( 'Add Custom Size', 'folie' ); ?></th>
							<td>
								<fieldset>
									<label>
										<input id="cl_custom_img_sizes" type="text" name="codeless_image_sizes[cl_custom_img_sizes]" value="">
									
											<p class="description"><?php esc_html_e( 'Here you can add a new id for a custom image size. You can edit width, height after Save. The new size will be shown at Other Section. After setting the new image size you can use it over your webpage.', 'folie' ); ?></p>
									</label>
								</fieldset>
							</td>
						</tr>
						...
						foreach ( $sizes as $size => $args ) : ?>
							...
							<tr valign="top" data-codeless-section="<?php echo esc_attr( $section ); ?>" style="display:none;">
								...
								<td>
									<label for="<?php echo esc_attr( $width ); ?>"><?php esc_html_e( 'Width', 'folie' ); ?></label>
									...
									<label for="<?php echo esc_attr( $height ); ?>"><?php esc_html_e( 'Height', 'folie' ); ?></label>
									...
									<label for="<?php echo esc_attr( $crop ); ?>"><?php esc_html_e( 'Crop Location', 'folie' ); ?></label>
			...
			<script>
				( function( $ ) {
					...
					$( '.codeless-image-sizes-admin-tabs a' ).click( function() {
