...
class Wp_Twitter_Api {
	...
	private function get_bearer_token() {
		...
		if ( is_wp_error( $response ) || 200 != $response['response']['code'] )
			...
			return $this->bail( __( 'Can\'t get the bearer token, check your credentials', 'folie' ), $response );
	...
	public function query( $query, $query_args = array(), $stop = false ) {
		...
		if ( is_wp_error( $response ) || 200 != $response['response']['code'] ){
			...
			if ( !$stop ){
				...
				return $this->bail( __( 'Bearer Token is good, check your query', 'folie' ), $response );
